...
const SCOPE = process.env.ZOHO_SCOPE || 'ZohoBooks.projects.CREATE,ZohoBooks.projects.READ,ZohoBooks.settings.READ';
const ZOHO_BASE = 'https://accounts.zoho.com/oauth/v2';

const authUrl = (scope, clientId, redirectUri) => {
  return `${ZOHO_BASE}/auth?scope=${scope}&client_id=${clientId}&response_type=code&access_type=offline&prompt=consent&redirect_uri=${redirectUri}`;
}

const tokenUrl = (code, clientId, clientSecret, redirectUri) => {
  return `${ZOHO_BASE}/token?code=${code}&redirect_uri=${redirectUri}&client_id=${clientId}&client_secret=${clientSecret}&grant_type=authorization_code&access_type=offline`;
}
    ...
    if (res.statusCode === 200) {
      ...
      res.on('end', () => {
        try {
          ...
          //const jsonString = JSON.stringify(tokenData, null, '  ')
          process.stdout.write(tokenData['access_token']+'#'+tokenData['refresh_token']);
